...
**Persistence standard** (corrected 2026-06-12; the earlier "Core, NOT ORM" dogma was an AI overstatement, not a project decision): repositories build on `backend/shared/persistence/`, and **all queries go through SQLAlchemy 2.0 — never hand-written raw SQL strings scattered across modules** (that re-implements param binding, row mapping, and dialect portability the library already provides). Within SQLAlchemy, pick the layer per domain: **ORM with typed mapped dataclasses is the default for new relational domains** (e.g. clinician signal, pathways) where it cuts row-mapping boilerplate; **Core expressions remain the right tool** for queue claiming (`FOR UPDATE SKIP LOCKED`), bulk operations, and reporting queries. Existing Core-based domains (`account/`, `content/`, `research_queue/`) stay as they are — no retroactive migration. Keep one `MetaData` and alembic as the single source of schema truth; don't mix ORM and Core styles inside one domain. Reserve raw `text()` SQL for a genuine optimization that's hard to express otherwise (a window function, a recursive CTE), behind one repository method, parameterized, tested, with a comment justifying it.
