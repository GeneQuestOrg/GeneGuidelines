...
class _Therapy(BaseModel):
    ...
    pmids: list[str] = Field(
        ...
        description="PubMed IDs of the reviews that support this therapy line.",
    )
...
# support a specific therapy. Used by both backfill helpers to avoid assigning a
# monitoring/assessment entry a citation that is really about a drug.
...
def _specific_keywords(name: str) -> list[str]:
    """Return keywords from a therapy name that are long and specific enough to
    anchor a PubMed citation. Generic clinical terms are excluded."""
    return [
        w.lower()
...
def _persist_therapies(disease_slug: str, therapies: list[_Therapy]) -> int:
    ...
    cur = conn.cursor()
    try:
        # If the disease already has therapies (e.g. from seed data), only merge
        # newly discovered PMIDs onto matching rows — don't insert LLM-generated
        # duplicates. Seed data is curated and LLM names rarely match exactly, so
        # inserting them previously created duplicates (e.g. "bisphosphonates" vs
        # "Bisphosphonates (pamidronate, zoledronate)"). New therapy lines should
        # be added via the seed file or a manual content PR.
        cur.execute(
            "SELECT id, name, pmids_json FROM therapies WHERE disease_slug = %s",
            (disease_slug,),
        )
