...
async def start_agent_run(
    ...
    *,
    ...
    profile_norm = (profile or "").strip().lower() or get_effective_default_model_profile()
    if profile_norm not in MODEL_PROFILES:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown profile '{profile or profile_norm}'. Allowed: {sorted(MODEL_PROFILES.keys())}",
        )
    loop = asyncio.get_event_loop()
    ticket = await loop.run_in_executor(None, lambda: db.get_ticket_by_id(ticket_id))
    if not ticket:
