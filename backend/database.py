...
def init_db():
    ...
    try:
        from .account_store import ensure_account_tables_schema
    except ImportError:
        from account_store import ensure_account_tables_schema  # type: ignore[no-redef]
...
"""
    ...
    pubmed_nodes = [
        ...
        },
        {
            ...
            "prompt": "You are an evidence appraisal assistant.\n\nBased ONLY on the evidence cards, article corpus, and metadata below, estimate credibility of the evidence base for clinical decision support.\n\nDisease/topic: {{ context.pm-2.result.query_text }}\nArticles in this prompt: {{ context.pm-2.result.article_count }}\nTotal analyzed (retrieval): {{ context.pm-2.result.total_analyzed }}\nBrowser fallback used: {{ context.pm-2.result.fallback_used }}\nPer-domain counts: {{ context.pm-2.result.per_domain_pmid_counts }}\nRecency distribution: {{ context.pm-2.result.recency_distribution }}\nCorpus capped for API limits: {{ context.pm-2.result.articles_text_corpus_capped }}\n\n--- Evidence cards ---\n{{ context.pm-2.result.evidence_cards }}\n\n--- Articles ---\n{{ context.pm-2.result.articles_text }}\n\nReturn structured assessment:\n1. evidence_score (0-100)\n2. evidence_level: high/moderate/low/very_low\n3. confidence_index (0-100)\n4. risk_of_bias\n5. main_limitations\n6. consistency_assessment\n7. clinical_reliability_comment\n\nDo not invent study details not present in the input.",
            "position_x": 0,
            "position_y": 520,
            "prompt_mode": "simple",
